...
namespace CarritoCompras.Service
{
    public class ApiServiceFirebase
    {
        ...
        public static async Task<bool> AgregarAlCarrito(Bolsa oBolsa)
        {
            Usuario oObject = new Usuario();
            try
            {
        ...
        public static async Task<Dictionary<string, Bolsa>> ObtenerCarrito()
        {
        ...
        public static async Task<List<Departamento>> ObtenerDepartamentos()
        {
            ...
            {
                HttpClient client = new HttpClient();
                string apiformat = string.Concat(AppSettings.ApiFirebase, "ubicacion/departamento.json?auth={0}");
        ...
        public static async Task<List<Provincia>> ObtenerCiudades(string p_nombredepartamento)
        {
            ...
            {
                HttpClient client = new HttpClient();
                string apiformat = string.Concat(AppSettings.ApiFirebase, "ubicacion/ciudad.json?auth={0}");
                var response = await client.GetAsync(string.Format(apiformat, AppSettings.oAuthentication.IdToken));
                if (response.StatusCode.Equals(HttpStatusCode.OK))
                {
                    var jsonstring = await response.Content.ReadAsStringAsync();
        ...
        public static async Task<List<Distrito>> Obtenerbarrios(string p_nombreprovincia)
        {
            ...
            {
                HttpClient client = new HttpClient();
                string apiformat = string.Concat(AppSettings.ApiFirebase, "ubicacion/barrio.json?auth={0}");
                var response = await client.GetAsync(string.Format(apiformat, AppSettings.oAuthentication.IdToken));
                if (response.StatusCode.Equals(HttpStatusCode.OK))
                {
                    var jsonstring = await response.Content.ReadAsStringAsync();
                    ...
                    if (jsonstring != "null")
                    {
                        ...
                        foreach (KeyValuePair<string, Compra> item in oObject)
                        {
                            oListaCompra.Add(new Compra()
                            {
                                ...
                                precioTotal = item.Value.precioTotal
                            });
                        }
                    }
